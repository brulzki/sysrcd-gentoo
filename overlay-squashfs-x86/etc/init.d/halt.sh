...
RC_NO_UMOUNTS=${RC_NO_UMOUNTS:-^(/|/dev|/dev/pts|/lib/rcscripts/init.d|/proc|/proc/.*|/sys|/newroot|/livemnt|/livemnt/memory|/livemnt/backstore|/livemnt/squashfs|/livemnt/isostore|/livemnt/boot)$}
...
remaining=$(awk '!/^#/ && $1 ~ /^\/dev\/loop/ && $2 != "/" {print $2}' /proc/mounts | \
            sort -r | grep -v -E "${RC_NO_UMOUNTS}" | grep -v '/livemnt')
[[ -n ${remaining} ]] && {
	sig=
	retry=3
	...
	while [[ -n ${remaining} && ${retry} -gt 0 ]]; do
		if [[ ${retry} -lt 3 ]]; then
			...
			eend $? "Failed to unmount filesystems this retry ${remaining}"
		else
			ebegin "Unmounting loopback filesystems"
			umount -d ${remaining} &>/dev/null
			eend $? "Failed to unmount filesystems ${remaining} "
		fi

		remaining=$(awk '!/^#/ && $1 ~ /^\/dev\/loop/ && $2 != "/" {print $2}' /proc/mounts | \
		            sort -r | grep -v -E "${RC_NO_UMOUNTS}")
		[[ -z ${remaining} ]] && break
...
for x in $(awk '{print $2}' /proc/mounts | sort -ur) ; do
	...
	if echo "${x}" | grep -q -E "${RC_NO_UMOUNTS}"
	then
		continue
	fi
...
mount_readonly() {
	...
	for x in $(awk '{print $2}' /proc/mounts | sort -ur) ; do
		...
		# Do not umount these ... will be different depending on value of CDBOOT
		# ====> livecd: continue even if "${x}" == "/" to avoid the error message
		if [[ -n $(echo "${x}" | grep -E "${RC_NO_UMOUNTS}") ]] ; then
			continue
		fi

		# Do not umount these ... (mounts done before chroot)
		if echo "${x}" | grep -q -E '/newroot|/boot|/livemnt/boot|/livemnt/memory|/livemnt/backstore|/livemnt/squashfs|/livemnt/isostore'
		then
			continue
		fi
...
# ---- sysresccd: special umounts
if cat /proc/mounts | awk '{print $2}' | grep -q '/livemnt/backstore'
then
	cmd="mount -o remount,mod:/livemnt/memory=ro /"
	${cmd} 2>/dev/null
	#echo "${cmd} --> $?"
	
	cmd="mount -f -o remount,ro /livemnt/memory"
	${cmd} 2>/dev/null
	#echo "${cmd} --> $?"
	
	cmd="umount -fl /livemnt/memory"
	${cmd} 2>/dev/null
	#echo "${cmd} --> $?"
	
	sleep 1
	
	cmd="mount -fl -o remount,ro /livemnt/backstore"
	${cmd} 2>/dev/null
	#echo "${cmd} --> $?"
fi
