...
src_unpack()
{
	...
	ln -s linux-${KV} linux
	cd linux-${KV}
	epatch ${FILESDIR}/alt-sources-3.1-01-stable-3.1.5.patch.bz2 || die "alt-sources stable patch failed."
	epatch ${FILESDIR}/alt-sources-3.1-02-fc16.patch.bz2 || die "alt-sources fedora patch failed."
	...
	sed -i -e "s/${oldextra}/EXTRAVERSION = -alt241/" Makefile
}
